...
function loadPriceImpactSettings() {
  return new Promise((resolve) => {
    ...
    db.get(
      ...
      ['price_impact_settings'],
      (err, row) => {
        if (!err && row) {
          try {
            priceImpactSettings = { ...priceImpactSettings, ...JSON.parse(row.value) };
            console.log('價格影響設置已載入:', priceImpactSettings);
          } catch (e) {
            console.error('解析價格影響設置失敗:', e);
          }
        }
        resolve();
      }
...
async function initializeStockPrice() {
  ...
  return new Promise((resolve, reject) => {
    ...
    db.get(
      "SELECT price FROM stock_prices ORDER BY timestamp DESC LIMIT 1",
      [],
      (err, row) => {
        if (err) {
          console.error('載入最新股價失敗:', err.message);
          resolve(currentPrice);
        } else if (row) {
          currentPrice = row.price;
          console.log(`載入最新Orange股價: $${currentPrice}`);
          resolve(currentPrice);
        } else {
          resolve(currentPrice);
        }
